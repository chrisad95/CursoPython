...
#variable_clave = "categoria_seguridad_alimentaria"
...
# Filtrar los datos de modo que se pueda incluir solo niños con sexo masculino y que tengan valores válidos en la columna 'categoria_seguridad_alimentaria'
datos_hombre_categoria_seguridad_alimentaria = datos[(datos['sexo'] == 'Hombre') & (datos['categoria_seguridad_alimentaria'].isin(['Inseguridad alta', 'Inseguridad leve', 'Inseguridad media', 'Seguridad']))]

# Calcular el conteo de niños por cada categoría de la variable 'tipo_de_piso'
conteo_categoria_seguridad_alimentaria = datos_hombre_categoria_seguridad_alimentaria['categoria_seguridad_alimentaria'].value_counts()
# Mostrar los resultados
print("Conteo de niños por categoría de 'categoria_seguridad_alimentaria':")
print(conteo_categoria_seguridad_alimentaria)
##################################################################################################################
...
# Convertir la variable categórica categoria_seguridad_alimentaria  en binaria
datos_limpios['categoria_seguridad_alimentaria_binario'] = datos_limpios['categoria_seguridad_alimentaria'].apply(lambda x: 1 if x == 'Seguridad' else 0)


# Filtrar los datos para incluir solo niños con sexo masculino y que tengan valores válidos en la columna 'tipo_de_piso'
datos_hombre_categoria_seguridad_alimentaria = datos_limpios[(datos_limpios['sexo'] == 'Hombre') & (datos_limpios['categoria_seguridad_alimentaria_binario'] == 1)]

# Seleccionar las variables relevantes
variables = ['n_hijos', 'region', 'sexo', 'condicion_empleo', 'categoria_seguridad_alimentaria_binario']

# Filtrar los datos para las variables seleccionadas y eliminar filas con valores nulos en esas variables
for i in variables:
    datos_hombre_categoria_seguridad_alimentaria = datos_hombre_categoria_seguridad_alimentaria[~datos_hombre_categoria_seguridad_alimentaria[i].isna()]
# Agrupar los datos por sexo y categoria_seguridad_alimentaria que posee
# contar el número de niños en cada grupo

conteo_ninos_por_categoria_seguridad_alimentaria = datos_hombre_categoria_seguridad_alimentaria.groupby(["sexo", "categoria_seguridad_alimentaria_binario"]).size()

print("Conteo de niños por categoría de 'tipo_de_piso':")
print(conteo_ninos_por_categoria_seguridad_alimentaria)
...
y = datos_dummies["categoria_seguridad_alimentaria_binario"]
...
#INTERPRETACION
#El coeficiente estimado para la variable n_hijos es negativo (-0.7329), lo cual indica que hay una asociación negativa entre el número de hijos y la razón de probabilidad de tener una cateogria de seguridad almentaria segura.
...
#para sugerir que el coeficiente asociado a n_hijos es diferente de cero. En otras palabras, parece haber una asociación significativa entre el número de hijos y la probabilidad de tener  una cateogria de seguridad alimentaria segura.
...
##########################################################################################################################################################################################################################
#RESPUESTA FINAL: 
#Cuando se utiliza el conjunto de datos filtrado, la precisión promedio del modelo disminuye a 0.5685789167152009 en comparación con el valor anterior. Por el lado de la distribución de los coeficientes beta en comparación al ejercicio previo, 
